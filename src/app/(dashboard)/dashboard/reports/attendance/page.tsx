...
export default function AttendancePage() {
    ...
    const specialties = useMemo(() => {
        return ['All', ...Array.from(new Set(mockDoctors.map(d => d.specialty)))];
    }, []);
    ...
    const attendanceData = useMemo(() => {
        ...
        return mockRoster.filter(record => {
    ...
    }, [startDate, endDate, searchTerm, selectedSpecialty]);

    const handleExport = () => {
        const data = attendanceData.map(r => ({
            'التاريخ': r.date,
            'اسم الطبيب': r.doctor?.fullNameArabic,
